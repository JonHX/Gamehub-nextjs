...
const HandTrackingCanvas = () => {
    ...
    const handLandmarkerRef = useRef(null); // Store hand landmarker instance
    const fingerColorsRef = useRef(["#FF5733", "#33FF57"]); // Ref to store finger colors (two colors for two hands)
    const drawnColors = useRef([]); // Ref to store drawn colors on canvas
    const brushSizeRef = useRef(20); // Default brush size (no state, to avoid re-renders)
        ...
        const detectHands = () => {
            if (videoRef.current && videoRef.current.readyState >= 2) {
                ...
                if (detections.landmarks) {
                    drawPointer(detections.landmarks, detections.hands ? detections.hands.length : 0);
                }
            }
